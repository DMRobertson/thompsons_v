...
Let :math:`S` be a set of words in :math:`V_{n,r}`. We can obtain new words by applying the operations :math:`\alpha_i` and :math:`\lambda` to elements of :math:`S`. Next, we can apply the operations to the new words we've received. This process continues indefinitely, producing a set of words :math:`T` whose elements are sourced from the original set :math:`S`. We call :math:`T` the *subalgebra generated by* :math:`S` and say that :math:`S` *generates* :math:`T`.
