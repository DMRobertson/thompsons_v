...
Because an Automorphism can be :meth:`repeatedly applied <Automorphism.__pow__>`, we may consider the orbit :math:`\{w\psi^n \mid n \in \mathbb{Z}\}` of any word :math:`w`. (Note that this is the orbit of :math:`w` under the cyclic subgroup $\langle \psi \rangle$, rather than all of $G_{n,r}$.)
