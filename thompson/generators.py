...
class Generators(list):
	...
	def descendants_above(self, floor):
		r"""Suppose we have a basis *floor* below the current basis *ceiling*. There are a finite number of elements below *ceiling* which are not below *floor*. This method enumerates them.
...
def rewrite_set(set, basis, new_basis):
	"""Maps *set* into the algebra generated by *new_basis* according to the bijection between *basis* and *new_basis*."""
