...
class Generators(list):
	...
	:ivar relabeller: If this generating set came from another parent generating set, this can be used to map back towards the parent.
	"""
	...
	def set_relabeller(self, relabeller):
		if self.signature != relabeller.domain.signature:
			raise ValueError("Relabeller domain signature {} does not match generating set signature {}.".format(
			  relabeller.domain.signature, self.signature))
		self.relabeller = relabeller
	
	def relabel(self):
		return self.relabeller.image_of_set(self)
